...
include "config.php"; 
$artist_id = $_GET["id"];
$sql_statement = "SELECT * FROM artist WHERE a_id ='$artist_id'";
...
while($row = mysqli_fetch_assoc($result)) { // Iterating the result
  ...
  $sql_statement = "SELECT * FROM creating NATURAL JOIN music WHERE a_id ='$artist_id'";
    $result = mysqli_query($db, $sql_statement);
            while($row = mysqli_fetch_assoc($result)) { // Iterating the result
